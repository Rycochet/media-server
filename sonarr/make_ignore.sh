...
#   IGNORE_DEPTH=2          # 2 levels total
#   IGNORE_FILE=.stfolders  # Not automatically shared
#   IGNORE_ROOT=/data/tv    # Path inside docker, split multiple paths with ":"
#
# Installation
#
...
IGNORE_ROOT="${IGNORE_ROOT:-/data/tv}"

declare -a valid_events=("Download" "Rename" "EpisodeFileDelete" "SeriesDelete" "Test")

for eventtype in "${valid_events[@]}"; do
    if [ "$eventtype" = "$sonarr_eventtype" ]; then
...
echo "make_ignore.sh: Unknown event type - $sonarr_eventtype"
