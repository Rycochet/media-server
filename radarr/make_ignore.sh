...
#   IGNORE_DEPTH=1              # 1 level total
#   IGNORE_FILE=.stfolders      # Not automatically shared
#   IGNORE_ROOT=/data/movies    # Path inside docker, split multiple paths with ":"
#
# Installation
#
...
IGNORE_ROOT="${IGNORE_ROOT:-/data/movies}"

declare -a valid_events=("Download" "Rename" "Test")

for eventtype in "${valid_events[@]}"; do
    if [ "$eventtype" = "$radarr_eventtype" ]; then
...
echo "make_ignore.sh: Unknown event type - $radarr_eventtype"
